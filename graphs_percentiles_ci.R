...
rho1b<--1.25
rho2b<--.6
dbs$pn1_red  <- ifelse(dbs$x < rho1b, dbs$pn1, NA)
dbs$pn5_blue <- ifelse(dbs$x >= rho1b & dbs$x <= rho2b, dbs$pn5, NA)
dbs$pn3_green<- ifelse(dbs$x > rho2b, dbs$pn3, NA)
...
graph<-ggplot(dbs) +
  ...
  geom_ribbon(aes(x = x, ymin = 0, ymax = pn1_red),
              fill = "firebrick", alpha = 0.25) +
  
  geom_ribbon(aes(x = x, ymin = 0, ymax = pn5_blue),
              fill = "navyblue", alpha = 0.25) +
  
  geom_ribbon(aes(x = x, ymin = 0, ymax = pn3_green),
